...
;(function(Action) {
  Action["SWITCH_MODE"] = "SWITCH_MODE"
  Action["CLOSE_RESULT"] = "CLOSE_RESULT"
  Action["SCAN_BACKSIDE"] = "SCAN_BACKSIDE"
  Action["SKIP_BACKSIDE"] = "SKIP_BACKSIDE"
  Action["CLOSE_WARNING"] = "CLOSE_WARNING"
})(Action || (Action = {}))
...
export function onModeSwitched(buttonElement) {
  elements.selector.querySelector("button.active")?.classList.remove("active")
  buttonElement.classList.add("active")
}

export function confirmScanningBackside(capturedId) {
...
function getDOMOfFormattedValue(value) {
...
function getDOMForLabel(labelText) {
  const paragraph = document.createElement("p")
...
function getFragmentForLabelAndValue(labelText, value) {
...
function getFragmentForFields(fields) {
  const fragment = document.createDocumentFragment()
...
export function showResult(capturedId) {
  let header = ""
  ...
  if (capturedId.idImageOfType(SDCId.IdImageType.Face) != null) {
    ...
    faceImage.src = `data:image/png;base64,${capturedId.idImageOfType(
      SDCId.IdImageType.Face
    ) ?? ""}`
